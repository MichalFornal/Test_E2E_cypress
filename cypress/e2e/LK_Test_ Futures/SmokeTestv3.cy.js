...
describe('sprawdzenie obecnosci', () => {
  ...
  it("Wiadomosci change user",() => {
    cy.wait(1000);
    cy.get('.chats-column > :nth-child(1) > :nth-child(3)').click();
    cy.wait(1000);
    cy.get('.chats-column > :nth-child(1) > :nth-child(2)').click();
    
    })
  it("Wiadomosci send text",() => {
    cy.get('.conversation-input-textarea').type(data.text);
    cy.get('.conversation-input-wrap > :nth-child(4)').click();
    cy.wait(1000)
    })
   }) 
